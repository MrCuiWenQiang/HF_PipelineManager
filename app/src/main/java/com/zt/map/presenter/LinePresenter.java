...
import com.zt.map.entity.db.system.Sys_qsdw;
...
//    private String[] directions;
    ...
    public void querySzwz(final long typeid) {
        if (szwz != null) {
            getView().querSzwz(szwz);
            return;
        }
       DBThreadHelper.startThreadInPool(new DBThreadHelper.ThreadCallback<List<Sys_szwz>>() {
           @Override
           protected List<Sys_szwz> jobContent() throws Exception {
               return  LitPalUtils.selectWhere(Sys_szwz.class);
           }

           @Override
           protected void jobEnd(List<Sys_szwz> sys_szwzs) {
                if (sys_szwzs!=null&&sys_szwzs.size()>0){
                    List<String> datas = new ArrayList<>();
                    for (Sys_szwz item : sys_szwzs) {
                        datas.add(item.getValue());
                    }
                    szwz = datas.toArray(new String[datas.size()]);
                    getView().querSzwz(szwz);
                }else {
                    getView().fail("未获取到选择数据");
                }
    ...
    public void queryQsdw(final long typeid) {
        if (qsdw != null) {
            getView().querQsdw(qsdw);
            return;
        }
       DBThreadHelper.startThreadInPool(new DBThreadHelper.ThreadCallback<List<Sys_qsdw>>() {
           @Override
           protected List<Sys_qsdw> jobContent() throws Exception {
               return  LitPalUtils.selectWhere(Sys_qsdw.class);
           }

           @Override
           protected void jobEnd(List<Sys_qsdw> sys_szwzs) {
                if (sys_szwzs!=null&&sys_szwzs.size()>0){
                    List<String> datas = new ArrayList<>();
                    for (Sys_qsdw item : sys_szwzs) {
                        datas.add(item.getValue());
                    }
                    szwz = datas.toArray(new String[datas.size()]);
                    getView().querQsdw(szwz);
                }else {
                    getView().fail("未获取到选择数据");
                }
           }
