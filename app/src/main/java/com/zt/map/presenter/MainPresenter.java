...
import com.zt.map.util.out.POIExcelUtil;
...
//                int count = LitPalUtils.selectCount(Tab_Marker.class, "projectid =? AND cldh not null", String.valueOf(marker.getProjectId())) + 1;
            ...
            protected void jobEnd(Boolean aBoolean) {
                getView().updateKillMaker();
                queryMeasureProject(false,marker.getProjectId());
            }
        });
    }
            ...
            protected Tab_Project jobContent() throws Exception {
                ...
                if (clName != null) {
                    if (!TextUtils.isEmpty(tab_project.getMeasureName())) {
                        int nowValue = Integer.valueOf(clName.toString());//新值不能比老值小
                        int oldValue = Integer.valueOf(tab_project.getMeasureName());
    ...
    public void queryProject(final long projectId) {
        ...
        DBThreadHelper.startThreadInPool(new DBThreadHelper.ThreadCallback<Map<String, Object>>() {
            ...
            protected void jobEnd(Map<String, Object> map) {
                ...
                long xendTime = System.currentTimeMillis();//结束时间
                float xexcTime = (float) (xendTime - endTime) / 1000;
                LogUtil.e("timer", "渲染结束++++" + xexcTime);
            }
        });
    }

    @Override
    public void queryMeasureProject(final boolean isclean, final long projectId) {
...
//                List<Tab_Marker> markers = LitPalUtils.selectWhere(Tab_Marker.class, "projectId = ? order by updateTime", String.valueOf(projectId));
                List<Tab_Marker> markers;
                if (EventDrive.isAddMarker()) {
                    List<Long> ids = EventDrive.getaddMarker();
                   markers = BatchDBUtil.whereMarkers("projectId = " + String.valueOf(projectId) + getMarkerSqls(ids));
                    EventDrive.clean();
                }else {
                    markers = BatchDBUtil.whereMarkers("projectId = "+String.valueOf(projectId) );
                }
            ...
            protected void jobEnd(List<Tab_Marker> datas) {
                if (getView() != null) {
                    getView().queryMeasure(isclean,datas);
                }
            }
        });
    }
...
//                e1.initExcel(table_maker_title, filePath, m_fileName, table_maker_names);
//                e1.writeObjListToExcel(markers, makerPath, mContext,table_maker_names);
                e1.writeObjListToExcelddd(markers, makerPath, mContext,table_maker_names);

                String l_fileName = table_line_title + ".xls";
                String linePath = filePath + "/" + l_fileName;
                POIExcelUtil e2 = new POIExcelUtil();
//                e2.initExcel(table_line_title, filePath, l_fileName, table_line_names);
//                e2.writeObjListToExcel(lines, linePath, mContext,table_line_names);
                e2.writeObjListToExcelddd(lines, linePath, mContext,table_line_names);
